...
public class KfDisplayTester implements DragMouseAdapter.Listener {
    ...
    public KfDisplayTester(FragmentActivity parentActivity, GLWindow gl_window, GameConfig gameConfigToLoad) {
    ...
    private void display(ArchiveEntry skeletonNifFile, ArrayList<String> skinNifFiles2, ArchiveEntry kff) {
        ...
        if (kff != null) {
            idleAnimations.add(kff.toString());
        }

        // now add the root to the scene so the controller sequence is live
        NifCharacter nifCharacter = new NifCharacter(skeletonNifFile.toString(), skinNifFiles2, mediaSources, idleAnimations);
    ...
    private void displayTes3(ArchiveEntry skeletonNifFile, ArrayList<String> skinNifFiles2) {
        modelGroup.removeAllChildren();
        ...
        nifCharacterTes3 = new NifCharacterTes3(skeletonNifFile.toString(), attachFileNames, mediaSources);
